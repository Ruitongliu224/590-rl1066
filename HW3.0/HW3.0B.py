...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct  5 18:22:00 2021
...
def NN_eval(x,submatrices):
    if(activation == "TANH"):       
        for i in range(0, len(submatrices),2):
            x = np.tanh(np.matmul(x,submatrices[i].transpose()+submatrices[i+1]))
    return x
...
if (model_type== "ANN"):
	...
	po=np.random.uniform(2,max_rand_wb,size=NFIT)
	...
	def plot_0():
         ...
         plt.title("Training and Validation Loss: %s" %model_type)
	...
	def plot_1(xcol = 1, xla='x',yla='y'):
         ...
         plt.xlabel(xla, fontsize=18);	plt.ylabel(yla, fontsize=18); 	
         plt.legend()
         plt.show()
	...
	def plot_2(xla='y_data',yla='y_predict'):
         ...
         plt.title("Parity plot: %s" %model_type)
